...
def train_agent(env, agent, Args):
    ...
    actions_taken = 0
    state, _ = env.reset()
    for step in track(range(args.total_timesteps), description="[cyan]Running timesteps..."):
...
# Plotting 'a' on the left y-axis
ax1.errorbar(x, r, yerr=r_std, fmt='o-', color='blue', label='Rewards')
ax1.set_xlabel('Timesteps')
ax1.set_ylabel('Rewards', color='blue')
ax1.tick_params(axis='y', labelcolor='blue')

# Create a second y-axis for 'b'
ax2 = ax1.twinx()
ax2.errorbar(x, a, yerr=a_std, fmt='s-', color='red', label='Actions')
ax2.set_ylabel('Action', color='red')
ax2.tick_params(axis='y', labelcolor='red')

# Add a title and grid for clarity
plt.title(r"Training RL model for {n} $\times$ {n} grid".format(n = n))
